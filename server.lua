...
function isAccountNumberAvailable(number)
    local available = not MySQL.scalar.await("SELECT 1 FROM ayse_banking_accounts WHERE account_number = ?", {number})
    -- local available2 = not MySQL.scalar.await("SELECT 1 FROM ayse_banking_shared_accounts WHERE account_number = ?", {number})
...
function createPersonalAccount(characterId)
    ...
    MySQL.query.await("INSERT INTO `ayse_banking_accounts` (`owner`, `account_number`) VALUES (?, ?)", {characterId, number})
    return number
end
...
function getInvoices(number)
    ...
    local received = MySQL.query.await("SELECT * FROM `ayse_banking_invoices` WHERE `receiver_account` = ?", {number})
    ...
    local sent = MySQL.query.await("SELECT * FROM `ayse_banking_invoices` WHERE `sender_account` = ?", {number})
...
function payInvoice(invoice, source)
    local character = AyseCore.Functions.GetPlayer(source)
    if character.bank < invoice.amount then return end

    AyseCore.Functions.DeductMoney(invoice.amount, source, "bank", "Invoice paid to " .. invoice.sender_name .. ".")
    MySQL.query.await("UPDATE `ayse_banking_invoices` SET `status` = 'paid' WHERE `invoice_id` = ?", {invoice.invoice_id})
    TriggerClientEvent("Ayse_Banking:updateInvoices", source, getInvoices(activePlayersAccounts[source] and activePlayersAccounts[source].number))

    local senderCharacter, senderSource = refreshInvoiceSender(invoice.invoice_id)
    if senderSource then
        AyseCore.Functions.AddMoney(invoice.amount, senderSource, "bank", "Invoice paid by " .. invoice.receiver_name .. ".")
    else
...
function transactionHistory(account, update)
    local table = "ayse_banking_accounts"
    local history = MySQL.scalar.await("SELECT `history` FROM `ayse_banking_accounts` WHERE `account_number` = ?", {account})
    -- if not history then 
    --     table = "ayse_banking_shared_accounts"
    --     history = MySQL.scalar.await("SELECT `history` FROM `ayse_banking_shared_accounts` WHERE `account_number` = ?", {account})
    -- end
...
function refreshInvoiceSender(invoiceId)
    local invoiceSender = MySQL.scalar.await("SELECT `sender_account` FROM `ayse_banking_invoices` WHERE `invoice_id` = ?", {invoiceId})
    local invoiceCharacter = MySQL.scalar.await("SELECT `owner` FROM `ayse_banking_accounts` WHERE `account_number` = ?", {invoiceSender})
    for accountSource, accountInfo in pairs(activePlayersAccounts) do
        if accountInfo.number == invoiceSender then
            TriggerClientEvent("Ayse_Banking:updateInvoices", accountSource, getInvoices(accountInfo.number))
...
function getInvoiceReceiverName(account)
    for accountSource, accountInfo in pairs(activePlayersAccounts) do
        if accountInfo.number == account then
            local character = AyseCore.Functions.GetPlayer(accountSource)
            if not character then break end
            return ("%s %s"):format(character.firstName, character.lastName), accountSource
        end
    end

    local receiver = MySQL.scalar.await("SELECT `owner` FROM `ayse_banking_accounts` WHERE `account_number` = ?", {account})
...
RegisterNetEvent("Ayse:moneyChange", function(player, moneyType, amount, action, description)
    ...
    TriggerClientEvent("Ayse_Banking:updateHistory", player, history)
end)

RegisterNetEvent("Ayse:characterLoaded", function(character)
    local bank = MySQL.scalar.await("SELECT `account_number` FROM `ayse_banking_accounts` WHERE `owner` = ?", {character.id})
    if not bank then
    ...
    TriggerClientEvent("Ayse_Banking:bankInfo", character.source, bank, getInvoices(bank), transactionHistory(bank))
end)

RegisterNetEvent("Ayse_Banking:interactInvoice", function(interaction, id)
    local src = source
    ...
    if interaction == "pay" then
        for _, invoice in pairs(invoices.unpaid) do
            if invoice.invoice_id == id then
                local character = AyseCore.Functions.GetPlayer(src)
                if character.bank < invoice.amount then return end
    ...
    if interaction == "reject" then
        for _, invoice in pairs(invoices.requests) do
            if invoice.invoice_id == id then
                MySQL.query.await("DELETE FROM `ayse_banking_invoices` WHERE `invoice_id` = ?", {invoice.invoice_id})
                TriggerClientEvent("Ayse_Banking:updateInvoices", src, getInvoices(account))
    ...
    for _, invoice in pairs(invoices.requests) do
        if invoice.invoice_id == id then
            MySQL.query.await("UPDATE `ayse_banking_invoices` SET `status` = 'unpaid' WHERE `invoice_id` = ?", {invoice.invoice_id})
            TriggerClientEvent("Ayse_Banking:updateInvoices", src, getInvoices(account))
            refreshInvoiceSender(invoice.invoice_id)
            break
        end
...
RegisterNetEvent("Ayse_Banking:createInvoice", function(account, amount, due)
    local src = source
    ...
    local character = AyseCore.Functions.GetPlayer(src)
    MySQL.query.await("INSERT INTO `ayse_banking_invoices` (`sender_name`, `receiver_name`, `sender_account`, `receiver_account`, `amount`, `created`, `due_in`, `status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", {
    ...
    TriggerClientEvent("Ayse_Banking:updateInvoices", src, getInvoices(sender), true)
    if not receiverSource then return end
    TriggerClientEvent("Ayse_Banking:updateInvoices", receiverSource, getInvoices(account))
end)

lib.callback.register("Ayse_Banking:getInfo", function(source)
...
lib.callback.register("Ayse_Banking:action", function(source, action, amount)
    ...
    local success = AyseCore.Functions[action == "Deposit" and "DepositMoney" or action == "Withdraw" and "WithdrawMoney"](amount, source)
    return success
end)

lib.callback.register("Ayse_Banking:actionATM", function(source, amount)
    ...
    local success = AyseCore.Functions.WithdrawMoney(amount, source)
    return success
end)

lib.callback.register("Ayse_Banking:transferMoney", function(source, account, amount, message)
    ...
    for receiver, accountInfo in pairs(activePlayersAccounts) do
        if accountInfo.number == account then
            local character = AyseCore.Functions.GetPlayer(source)
            local receiverCharacter = AyseCore.Functions.GetPlayer(receiver)
            ...
            local success = AyseCore.Functions.TransferBank(amount, source, receiver, senderMessage, receiverMessage)
            if not success then
