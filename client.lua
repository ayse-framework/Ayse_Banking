...
RegisterNUICallback("interactATM", function(data)
    ...
    lib.callback("Ayse_Banking:actionATM", false, function(result)
...
RegisterNUICallback("interactInvoice", function(data)
    PlaySoundFrontend(-1, "PIN_BUTTON", "ATM_SOUNDS", 1)
    TriggerServerEvent("Ayse_Banking:interactInvoice", data.type:lower(), data.id)
end)

RegisterNUICallback("createInvoice", function(data)
    TriggerServerEvent("Ayse_Banking:createInvoice", data.account, tonumber(data.amount), data.due)
end)

RegisterNUICallback("transferMoney", function(data)
    lib.callback("Ayse_Banking:transferMoney", false, function(result)
        SendNUIMessage({
...
RegisterNUICallback("action", function(data)
    lib.callback("Ayse_Banking:action", false, function(result)
        local character = AyseCore.Functions.GetSelectedCharacter()
        SendNUIMessage({
            type = "action",
            personalAccountBalance = character.bank,
...
RegisterNetEvent("Ayse_Banking:updateInvoices", function(invoices, create)
    SendNUIMessage({
        type = "updateInvoices",
        invoices = invoices,
        create = create
...
RegisterNetEvent("Ayse_Banking:updateHistory", function(history)
...
RegisterNetEvent("Ayse_Banking:bankInfo", function(bank, invoices, history)
    local character = AyseCore.Functions.GetSelectedCharacter()
    SendNUIMessage({
        type = "bankInfo",
